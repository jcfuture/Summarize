<template>
	<div class="main-content" :style='{"padding":"10px 30px 30px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(64, 158, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('shuifei','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('shuifei','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>




					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 128, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('shuifei','月度用水统计')" type="warning" @click="chartDialog1()">月度用水统计</el-button>
				</el-row>
			</el-form>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yongshuiliangChart1" style="width:100%;height:600px;"></div>
...
export default {
  ...
  methods: {
    ...
    payHandler(row){
      this.$storage.set('paytable','shuifei');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        var yongshuiliangChart1 = echarts.init(document.getElementById("yongshuiliangChart1"),'macarons');
        this.$http({
            url: `shuifei/value/dengjiriqi/yongshuiliang/月`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yongshuiliangChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yongshuiliangChart1.resize();
                };
            }
        });
      })
