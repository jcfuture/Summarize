		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"3px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">维修需求</label>
						<el-input v-model="searchForm.weixiuxuqiu" placeholder="维修需求" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="维修状态" prop="weixiuzhuangtai">
						...
						<el-select  @change="weixiuzhuangtaiChange" clearable v-model="searchForm.weixiuzhuangtai" placeholder="请选择维修状态">
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(64, 158, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('weixiu','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('weixiu','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>


					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('weixiu','审核')" :disabled="dataListSelections.length <= 0" type="danger" @click="shBatchDialog()">批量审核</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					label="维修需求">
						<template slot-scope="scope">
							{{scope.row.weixiuxuqiu}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="sheshitupian" width="200" label="设施图片">
						<template slot-scope="scope">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="shenqingweixiushijian"
					label="申请维修时间">
						<template slot-scope="scope">
							{{scope.row.shenqingweixiushijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('weixiu','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
  ...
  methods: {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
           ...
           if(this.searchForm.weixiuxuqiu!='' && this.searchForm.weixiuxuqiu!=undefined){
            params['weixiuxuqiu'] = '%' + this.searchForm.weixiuxuqiu + '%'
          }
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
            params['sfsh'] = this.searchForm.sfsh
          }
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          weixiuxuqiu: row.weixiuxuqiu,
          sheshitupian: row.sheshitupian,
          sheshiweixiuyuanyin: row.sheshiweixiuyuanyin,
          shenqingweixiushijian: row.shenqingweixiushijian,
    ...
    shBatchHandler(){
      this.$confirm(`确定一键审核操作?`, "提示", {
        ...
        this.$http({
          url: "weixiu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
