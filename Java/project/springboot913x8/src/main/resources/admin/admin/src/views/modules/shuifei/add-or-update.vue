	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="用水量" prop="yongshuiliang">
					<el-input v-model="ruleForm.yongshuiliang" placeholder="用水量" clearable  :readonly="ro.yongshuiliang"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="用水量" prop="yongshuiliang">
...
export default {
	...
	computed: {
		...
		feiyong:{
			get: function () {
				return 1*this.ruleForm.yongshuiliang*this.ruleForm.danjia
			}
		},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shuifei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shuifeiCrossAddOrUpdateFlag = false;
						 this.parent.search();
