...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>申请标题</div>
        <el-input v-model="formSearch.shenqingbiaoti" placeholder="申请标题" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('pinkunshenqing','新增')" :style='{"cursor":"pointer","border":"1px solid #db961f","padding":"0px 15px","boxShadow":"1px 1px 3px #f8a412","margin":"0px 10px 0 0","color":"#fff","outline":"none","borderRadius":"4px","background":"#f8a412","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/pinkunshenqingAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"0 0 10px","background":"none","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"border":"1px solid #dfdfdf","padding":"20px 10px","margin":"16px 0 0 0","borderRadius":"16px","flexWrap":"wrap","background":"#fff","display":"flex","width":"1200px","height":"auto"}'>
			<div :style='{"margin":"0 1% 20px 1%","borderRadius":"8px","background":"none","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"cursor":"pointer","width":"100%","objectFit":"cover","borderRadius":"8px","display":"block","height":"274px"}' v-if="item.pinkuntupian && item.pinkuntupian.substr(0,4)=='http'" :src="item.pinkuntupian" class="image" />
				<img :style='{"cursor":"pointer","width":"100%","objectFit":"cover","borderRadius":"8px","display":"block","height":"274px"}' v-else :src="baseUrl + (item.pinkuntupian?item.pinkuntupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","boxShadow":"1px 1px 1px #ddd","margin":"4px 0 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"8px","background":"radial-gradient(circle, rgba(250,250,250,1) 0%, rgba(220,230,250,0.5) 100%)","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.shenqingbiaoti}}</div>
				<div :style='{"padding":"0 10px","boxShadow":"1px 1px 1px #ddd","margin":"4px 0 0 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"8px","background":"radial-gradient(circle, rgba(250,250,250,1) 0%, rgba(220,230,250,0.5) 100%)","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.shenqingshijian}}</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.shenqingbiaoti != '') searchWhere.shenqingbiaoti = '%' + this.formSearch.shenqingbiaoti + '%';
	      params['sfsh'] = '是';
        this.$http.get('pinkunshenqing/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/pinkunshenqingDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
